...
test('approval requests never masquerade as completed operations', () => {
  for (const code of ['RESOLVE', 'ACCEPT', 'ASSIGN', 'CLOSE', 'HOLD', 'ESCALATE', 'CANCEL', 'REOPEN', 'CONTROLLED_JUMP_REQUEST']) assert.equal(workflowOutcome(code), 'PENDING_APPROVAL')
})
test('collaboration confirmations remain pending while direct operations can complete', () => {
  ...
  for (const code of ['INTERNAL_COMMENT', 'CLAIM', 'TRANSFER', 'CLASSIFY', 'RESUME', 'REQUEST_USER_FEEDBACK']) assert.equal(workflowOutcome(code), 'COMPLETED')
})
