...
<template>
  <section class="panel detail-panel processing-knowledge" aria-label="处理参考知识">
    ...
    <template v-else>
      ...
      <ul v-if="visibleEntries.length"><li v-for="entry in visibleEntries" :key="entry.id"><RouterLink :to="`/knowledge/${encodeURIComponent(entry.id)}`" target="_blank" rel="noopener noreferrer">{{ entry.title }} ↗</RouterLink><small v-if="entry.serviceCatalogItemIds.includes(catalogId)">适用于当前服务目录</small></li></ul>
