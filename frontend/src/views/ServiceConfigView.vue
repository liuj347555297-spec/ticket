...
import SystemManagementTable from '@/components/SystemManagementTable.vue'
import OfferingManagementTable, { type OfferingManagementRow } from '@/components/OfferingManagementTable.vue'
...
const showSystemEditor = ref(false), showSystemDetail = ref(false), showSystemStatusDialog = ref(false)
const systemStatusAction = ref<'publish' | 'retire'>('publish'), systemStatusReason = ref('')
...
const offeringManagementRows = computed<OfferingManagementRow[]>(() => scopedOfferings.value.map(({ mapping, item }) => {
  const managed = item ? adminConfigs.value.find(config => config.id === item.id) : undefined
  return { mapping, item, managed: Boolean(managed), lifecycleStatus: managed?.lifecycleStatus ?? (item ? 'PUBLISHED' : 'UNKNOWN'), processConfigured: item && selectedId.value === item.id ? routingPolicies.value.length > 0 : undefined }
}))
const systemServiceCounts = computed<Record<string, number>>(() => selectedServiceSystem.value && mappingReady.value ? { [selectedServiceSystem.value.systemCode]: effectiveServiceMappings.value.length } : {})
...
function openSystemSettings(): void { if (setWorkspace('SETTINGS') && !creatingSystem.value && selectedServiceSystem.value) { resetServiceSystemEditor(selectedServiceSystem.value); showSystemEditor.value = true } }
async function openSystemFor(system: ServiceSystem, section: WorkspaceSection): Promise<void> {
  await selectServiceSystem(system)
  if (selectedServiceSystemCode.value !== system.systemCode) return
  if (section === 'SETTINGS') { resetServiceSystemEditor(system); workspaceSection.value = section; showSystemEditor.value = true; return }
  setWorkspace(section)
}
async function viewSystem(system: ServiceSystem): Promise<void> {
  await selectServiceSystem(system)
  if (selectedServiceSystemCode.value === system.systemCode) showSystemDetail.value = true
}
async function requestSystemStatus(system: ServiceSystem, action: 'publish' | 'retire'): Promise<void> {
  await selectServiceSystem(system)
  if (selectedServiceSystemCode.value !== system.systemCode) return
  systemStatusAction.value = action; systemStatusReason.value = ''; serviceSystemError.value = ''; showSystemStatusDialog.value = true
}
async function confirmSystemStatus(): Promise<void> {
  if (systemStatusReason.value.trim().length < 4) { serviceSystemError.value = '请填写至少 4 个字的变更说明。'; return }
  await changeSystemStatus(systemStatusAction.value, systemStatusReason.value, true)
  if (!serviceSystemError.value) showSystemStatusDialog.value = false
}
function editOfferingFromTable(item: ServiceCatalogItem): void { openOffering(item); if (adminConfigs.value.some(config => config.id === item.id)) openUpdate() }
function lifecycleOfferingFromTable(item: ServiceCatalogItem, action: 'publish' | 'retire' | 'approve'): void {
  const managed = adminConfigs.value.find(config => config.id === item.id)
  if (!managed) return
  selectAdmin(managed); workspaceSection.value = 'DETAIL'; openAction(action === 'publish' ? 'PUBLISH' : action === 'approve' ? 'APPROVE' : 'RETIRE')
}
...
function newSystem(): void { if (mutationBusy.value || !canManageForms.value || !canLeaveDesigner()) return; clearDesigner(); creatingSystem.value = true; clearOffering(); resetServiceSystemEditor(); workspaceSection.value = 'SETTINGS'; showSystemEditor.value = true }
function closeSystemEditor(): void {
  if (serviceSystemSaving.value) return
  showSystemEditor.value = false; creatingSystem.value = false
  if (selectedServiceSystem.value) resetServiceSystemEditor(selectedServiceSystem.value)
  else resetServiceSystemEditor()
  if (workspaceSection.value === 'SETTINGS') workspaceSection.value = 'SERVICES'
  serviceSystemError.value = ''
}
...
async function saveServiceSystem(): Promise<void> {
  ...
  try {
    ...
    if (alive(identity)) { workspaceSection.value = 'SERVICES'; showSystemEditor.value = false; serviceSystemNotice.value = '系统信息已保存；上架/下架需单独操作，保存信息不会发布下属服务。' }
...
async function changeSystemStatus(action: 'publish' | 'retire', suppliedReason?: string, confirmed = false): Promise<void> {
  if (!selectedServiceSystem.value || creatingSystem.value || !canManageForms.value || mutationBusy.value) return
  const system = selectedServiceSystem.value, reason = (suppliedReason ?? serviceSystemEditor.value.reason).trim(), identity = identityScope.current()
  if (reason.length < 4) { serviceSystemError.value = '请先在系统设置填写至少 4 字的变更说明。'; return }
  if (!confirmed && !window.confirm(`确认${action === 'publish' ? '上架' : '下架'}系统“${system.systemName}”？系统上架不会自动发布工单服务。`)) return
  serviceSystemSaving.value = true
  try {
    ...
    if (alive(identity)) { workspaceSection.value = 'SERVICES'; serviceSystemNotice.value = '系统上架状态已由服务端更新并审计；各服务仍独立发布。' }
...
async function reloadIdentity(): Promise<void> {
  ...
  showEditor.value = false; showActionDialog.value = false; showAnnouncementForm.value = false; showSystemEditor.value = false; showSystemDetail.value = false; showSystemStatusDialog.value = false; announcements.value = []
...
<template>
  <div class="page-heading form-admin-heading"><div><span class="eyebrow">统一配置入口</span><h2>系统服务配置</h2><p>新建或选择系统 → 配置工单服务 → 在当前系统内设计该服务的表单与流程。</p></div></div>
  ...
  <SystemManagementTable v-if="workspaceSection === 'SERVICES'" :systems="serviceSystems" :loading="serviceSystemsLoading" :error="serviceSystems.length ? '' : serviceSystemError" :warning="systemListWarning" :selected-code="selectedServiceSystemCode" :busy="mutationBusy || designerState.busy" :can-manage="canManageForms" :can-read-design="canReadDesign" :service-counts="systemServiceCounts" @refresh="loadServiceSystems()" @create="newSystem" @select="selectServiceSystem" @view="viewSystem" @edit="(system) => openSystemFor(system, 'SETTINGS')" @modules="(system) => openSystemFor(system, 'MODULES')" @mappings="(system) => openSystemFor(system, 'MAPPINGS')" @design="async (system) => { await selectServiceSystem(system); if (selectedServiceSystemCode === system.systemCode) openDesigner() }" @status="requestSystemStatus" />
  ...
  <p v-if="publishedMetadataNotice && workspaceSection === 'SERVICES'" class="form-alert" role="status">{{ publishedMetadataNotice }}</p>
  <OfferingManagementTable v-if="workspaceSection === 'SERVICES' && selectedServiceSystem" :rows="offeringManagementRows" :loading="adminLoading || modulesLoading || mappingLoading" :ready="mappingReady && adminReady" :error="serviceSystemError || (!adminReady && !adminLoading ? managementError : '')" :busy="mutationBusy" :can-manage="canManageForms" :can-read-design="canReadDesign" :inherited="mappingReady && inheritedServices" @view="openOffering" @edit="editOfferingFromTable" @lifecycle="lifecycleOfferingFromTable" @design="openDesigner" @mappings="setWorkspace('MAPPINGS')" @create="openDesigner()" />
  ...
  <section v-if="canManageForms && ['MODULES', 'MAPPINGS'].includes(workspaceSection) && selectedServiceSystem" class="panel service-system-registry-panel" :data-section="workspaceSection" :inert="mutationBusy">
  ...
  <el-dialog v-model="showSystemEditor" :title="creatingSystem ? '新建系统' : '编辑系统'" width="min(720px, calc(100vw - 32px))" :close-on-click-modal="!serviceSystemSaving" :close-on-press-escape="!serviceSystemSaving" :show-close="!serviceSystemSaving" @closed="closeSystemEditor">
    <el-form label-position="top" @submit.prevent="saveServiceSystem"><div class="system-dialog-grid"><el-form-item label="系统编码" required><el-input v-model.trim="serviceSystemEditor.systemCode" :disabled="!creatingSystem" maxlength="64" placeholder="例如 ERP" /></el-form-item><el-form-item label="系统名称" required><el-input v-model.trim="serviceSystemEditor.systemName" maxlength="200" /></el-form-item><el-form-item label="所属组织" required><el-input v-model.trim="serviceSystemEditor.owningOrganizationId" maxlength="128" placeholder="例如 ORG-IT" /></el-form-item><el-form-item label="系统负责人"><el-input v-model.trim="serviceSystemEditor.ownerIamUserId" maxlength="128" placeholder="负责人 ID" /></el-form-item><el-form-item label="CMDB CI"><el-input v-model.trim="serviceSystemEditor.ciId" maxlength="128" placeholder="可选" /></el-form-item><el-form-item label="状态"><el-input :model-value="serviceSystemEditor.lifecycleStatus === 'DRAFT' ? '草稿' : serviceSystemEditor.lifecycleStatus === 'PUBLISHED' ? '已上架' : '已下架'" disabled /></el-form-item><el-form-item class="system-dialog-wide" label="变更说明" required><el-input v-model.trim="serviceSystemEditor.reason" type="textarea" :rows="3" maxlength="500" show-word-limit placeholder="至少 4 个字，将写入审计" /></el-form-item></div><el-alert v-if="serviceSystemError" type="error" :closable="false" show-icon>{{ serviceSystemError }}</el-alert></el-form>
    <template #footer><el-button :disabled="serviceSystemSaving" @click="closeSystemEditor">取消</el-button><el-button type="primary" :loading="serviceSystemSaving" :disabled="!creatingSystem && selectedServiceSystem?.lifecycleStatus !== 'DRAFT'" @click="saveServiceSystem">保存</el-button></template>
  </el-dialog>
  <el-drawer v-model="showSystemDetail" title="系统详情" size="min(520px, 92vw)"><el-descriptions v-if="selectedServiceSystem" :column="1" border><el-descriptions-item label="系统编码">{{ selectedServiceSystem.systemCode }}</el-descriptions-item><el-descriptions-item label="系统名称">{{ selectedServiceSystem.systemName }}</el-descriptions-item><el-descriptions-item label="所属组织">{{ selectedServiceSystem.owningOrganizationId }}</el-descriptions-item><el-descriptions-item label="负责人">{{ selectedServiceSystem.ownerIamUserId || '未配置' }}</el-descriptions-item><el-descriptions-item label="CMDB CI">{{ selectedServiceSystem.ciId || '未关联' }}</el-descriptions-item><el-descriptions-item label="状态">{{ selectedServiceSystem.lifecycleStatus }}</el-descriptions-item><el-descriptions-item label="并发版本">v{{ selectedServiceSystem.version }}</el-descriptions-item><el-descriptions-item label="变更说明">{{ selectedServiceSystem.changeReason || '未提供' }}</el-descriptions-item></el-descriptions><template #footer><el-button @click="showSystemDetail = false">关闭</el-button><el-button v-if="canManageForms && selectedServiceSystem?.lifecycleStatus === 'DRAFT'" type="primary" @click="showSystemDetail = false; openSystemSettings()">编辑</el-button></template></el-drawer>
  <el-dialog v-model="showSystemStatusDialog" :title="systemStatusAction === 'publish' ? '上架系统' : '下架系统'" width="min(520px, calc(100vw - 32px))" :close-on-click-modal="!serviceSystemSaving" :show-close="!serviceSystemSaving"><p>确认{{ systemStatusAction === 'publish' ? '上架' : '下架' }}系统“{{ selectedServiceSystem?.systemName }}”？系统状态不会自动改变工单服务发布状态。</p><el-form label-position="top"><el-form-item label="变更说明" required><el-input v-model.trim="systemStatusReason" type="textarea" :rows="3" maxlength="500" show-word-limit /></el-form-item></el-form><el-alert v-if="serviceSystemError" type="error" :closable="false" show-icon>{{ serviceSystemError }}</el-alert><template #footer><el-button :disabled="serviceSystemSaving" @click="showSystemStatusDialog = false">取消</el-button><el-button type="primary" :loading="serviceSystemSaving" @click="confirmSystemStatus">确认{{ systemStatusAction === 'publish' ? '上架' : '下架' }}</el-button></template></el-dialog>
...
<style scoped>
.system-dialog-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:0 16px; }.system-dialog-wide { grid-column:1 / -1; }
@media(max-width:720px) { .system-dialog-grid { grid-template-columns:1fr; }.system-dialog-wide { grid-column:auto; } }
