...
public class AttachmentController {
    ...
    @GetMapping("/{attachmentId}/inline") ResponseEntity<InputStreamResource> inline(@PathVariable @Pattern(regexp="^TKT-[0-9]{8}-[0-9]{6}$") String ticketId,@PathVariable @Pattern(regexp="^ATT-[0-9a-fA-F-]{36}$") String attachmentId) { var result=service.download(ticketId,attachmentId); if (!result.attachment().detectedMediaType().startsWith("image/")) throw new cn.servicehub.attachment.application.AttachmentValidationException(); return ResponseEntity.ok().contentType(MediaType.parseMediaType(result.attachment().detectedMediaType())).contentLength(result.attachment().sizeBytes()).header(HttpHeaders.CONTENT_DISPOSITION,"inline").header(HttpHeaders.CACHE_CONTROL,"no-store, private").header("X-Content-Type-Options","nosniff").body(new InputStreamResource(result.content())); }
}
