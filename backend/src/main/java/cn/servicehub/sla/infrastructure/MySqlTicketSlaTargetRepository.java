...
public class MySqlTicketSlaTargetRepository implements TicketSlaTargetRepository {
    ...
    public java.util.List<String> findDueTicketIds(java.time.Instant fromInclusive, java.time.Instant toExclusive) { return jdbc.queryForList("SELECT ticket_id FROM ticket_sla_target WHERE resolved_at IS NULL AND ((first_responded_at IS NULL AND response_due_at>=? AND response_due_at<?) OR (resolution_due_at>=? AND resolution_due_at<?))", String.class, ts(fromInclusive), ts(toExclusive), ts(fromInclusive), ts(toExclusive)); }
