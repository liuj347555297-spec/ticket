...
public class InMemoryTicketSlaTargetRepository implements TicketSlaTargetRepository {
    ...
    public java.util.List<String> findDueTicketIds(java.time.Instant fromInclusive, java.time.Instant toExclusive) { return targets.values().stream().filter(target -> target.resolvedAt() == null && (dueToday(target.responseDueAt(), target.firstRespondedAt(), fromInclusive, toExclusive) || dueToday(target.resolutionDueAt(), target.resolvedAt(), fromInclusive, toExclusive))).map(TicketSlaTarget::ticketId).toList(); }
    ...
    private static boolean dueToday(java.time.Instant dueAt, java.time.Instant completedAt, java.time.Instant fromInclusive, java.time.Instant toExclusive) { return completedAt == null && dueAt != null && !dueAt.isBefore(fromInclusive) && dueAt.isBefore(toExclusive); }
}
