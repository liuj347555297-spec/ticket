...
public class MySqlNotificationRepository implements NotificationRepository {
    ...
    @Override @Transactional public boolean save(Notification n) {
        try {
            ...
            if ("WPS_IM".equals(n.payload().get("preferredChannel"))) jdbc.update("INSERT INTO notification_delivery (id, notification_id, channel, provider_channel_code, route_rule_id, status, attempt_count, last_error_code, created_at, updated_at) VALUES (?, ?, 'WPS_IM', ?, ?, 'SUPPRESSED', 0, 'CHANNEL_DISABLED', ?, ?)", "NDL-" + UUID.randomUUID(), n.id(), n.payload().get("providerChannelCode"), n.payload().get("routeRuleId"), ts(n.createdAt()), ts(n.createdAt()));
            jdbc.update("INSERT INTO message_outbox (id, aggregate_type, aggregate_id, event_type, payload, status, attempt_count, available_at, processed_at, created_at) VALUES (?, 'notification', ?, 'NOTIFICATION_CREATED', ?, 'PENDING', 0, ?, NULL, ?)", "OUT-" + UUID.randomUUID(), n.id(), json.writeValueAsString(n.payload()), ts(n.createdAt()), ts(n.createdAt()));
            return true;
